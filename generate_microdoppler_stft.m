...
addpath(genpath(''));%replace with your own path
% parameter setting
...
max_value = params.max_value; % normalization the maximum of data WITH 1642
...
M = 10; % number of frames for generating micro-Doppler image
...
WINDOW = 64; % STFT parameters
NOVEPLAP = 48; % STFT parameters
...
loop = 255;
Tx = 2;
folder_location = '';%repace with your raw bin file. No need to obtain a matlab type.
currentPos = 60;
% 提取从 currentPos 开始到末尾的字符串
subStr = folder_location(currentPos:end);
tokens = regexp(subStr, '^(.*?)_', 'tokens');
match = regexp(subStr, '_', 'once');
match = currentPos - 1 + match;
if ~isempty(tokens)
    resultStr1 = tokens{1}{1}; 
...
tokens = regexp(subStr, '^(.*?)_', 'tokens');
if ~isempty(tokens)
    resultStr2 = tokens{1}{1};
    resultStr2 = [' ',resultStr2];
end
filename =strcat(folder_location(58),'swing',resultStr1,'degrees',resultStr2,'rpm');
display(filename)
%rename the title of the heatmap
data_frames = readDCA1000(folder_location, samples);
data_length = length(data_frames);
data_each_frame = samples*loop*Tx;
...
% STFT operation
for h = 1:Lr*La
    [S,F,T] = spectrogram(data_conca(h,:), WINDOW, NOVEPLAP, 128, 1/Tc,'centered');
...
xlabel('time (s)')
ylabel('velocity (m/s)')
title(filename,'micro-Doppler map')
colorbar

% save_location = '';  % 指定保存的文件夹
% picture = strcat(filename,'.png');       % 指定文件名和后缀
% display(picture);
% fullFileName = fullfile(save_location, picture);
% 
% % 保存当前图形为 PNG 图片
